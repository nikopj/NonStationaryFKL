...
class LatentGPModel2D(gpytorch.models.ExactGP):
	def __init__(self, train_x, train_y, likelihood, mean=gpytorch.means.ZeroMean, grid=False):
		super(LatentGPModel2D, self).__init__(train_x, train_y, likelihood)
		self.mean_module = mean()
...
class NonStatSpectralModel(gpytorch.models.ExactGP):
	def __init__(self, train_x, train_y, likelihood, **kwargs):
		from .kernels import NonStatSpectralGPKernel
		super(NonStatSpectralModel, self).__init__(train_x, train_y, likelihood)
		self.mean_module = gpytorch.means.ConstantMean()
		self.covar_module = NonStatSpectralGPKernel(**kwargs)
		self.covar_module.initialize_from_data(train_x, train_y, **kwargs)
